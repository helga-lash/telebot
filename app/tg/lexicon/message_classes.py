...
class Messages:
    ...
    userNameNotValid: str = ('Имя должно содержать только русские буквы, начинаться с большой буквы'
                             ' и быть длинной от одного до десяти символов')
    userNameFirst: str = ('Для продолжения необходимо зарегистрироваться в системе.\n'
                          'Если Вы согласны введите свое имя, если нет нажмите кнопку "Нет".')
    userSurname: str = 'Введите свою фамилию.'
    userSurnameNotValid: str = ('Фамилия должно содержать только русские буквы, начинаться с большой буквы'
                                ' и быть длинной от одного до десяти символов')
    userPhone: str = 'Введите номер телефона для связи с Вами в формате "79*********".'
    userPhoneNotValid: str = ('Номер телефона должен содержать только цифры, начинаться с семерки '
                              'и быть длинной одиннадцать символов.')
    ...
    reserveNotes: str = 'Введите заметки для резервирования.'
    recordReserved: str = ('Вы хотите зарезервировать запись на {day} в {tm} со следующей заметкой:\n'
                           '"{notes}"?')
